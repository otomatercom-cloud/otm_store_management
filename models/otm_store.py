...
class OtmStore(models.Model):
    ...
    def _compute_stock_stats(self):
        ...
        for store in self:
            ...
            store.low_stock_count = len(quants.filtered(
                lambda q: q.product_tmpl_id.otm_reorder_qty and q.quantity <= q.product_tmpl_id.otm_reorder_qty
            ))
    ...
    def _cron_check_stock_alerts(self):
        ...
        for every store that currently has low-stock, near-expiry or
        expired items."""
        Quant = self.env['otm.stock.quant']
        ...
        for store in self.search([('status', '=', 'active')]):
            ...
            note = []
            if low:
                note.append(f'{len(low)} product(s) at or below reorder level.')
            if expiring:
